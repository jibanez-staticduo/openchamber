...
Linux desktop packaging starts with AppImage. Build it from the repository root with `bun run electron:build`; output is written to `packages/electron/dist`.

For a local current-user install or update, run `bun run --cwd packages/electron install:linux-appimage` after building. The script installs the AppImage under `~/.local/opt/openchamber/`, writes `~/.local/share/applications/openchamber.desktop`, installs the `openchamber` icon, and refreshes desktop/icon caches when available. It does not install system-wide packages.
